...
Graph * graph_addNode(Graph * g, const Node* n){
    ...
    for(i=0; i<g->numNodes; i++){
        if(ids[i]==node_getId(n)){
            printf("Id del nodo repetida, el grafo no se ha modificado con éxito.\n");
            return NULL;
        }
    }
    
    g->nodos[g->numNodes] = node_ini();
...
int graph_print(FILE *pf, const Graph * g){
    ...
    num = fprintf(pf, "N=%d, E=%d:\r\n", g->numNodes, graph_getNedges(g)); /*El \r\n es el comando para salto de línea en el bloc de notas de windows.*/
