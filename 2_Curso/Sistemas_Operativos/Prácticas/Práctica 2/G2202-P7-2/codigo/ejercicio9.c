...
int main(int argc, char **argv){
    ...
    for(i=1; i<=num_cash; i++){
        if((pid = fork())<0){ /* Error */
            ...
            if(i==1){
               if(Borrar_Semaforo(semid) == ERROR && pid!=0){ /* Error en el primer fork(), el padre borra semaforos */
                  printf("Advertencia: no se han podido eliminar los semaforos despues del error anterior\n");
               }
            }
            exit(EXIT_FAILURE);
    ...
    if(pid == 0){
        ...
        if(!in){ /* Error */
            ...
            free(name);
            free(fcash);
            /* Los semaforos son liberados por el proceso padre aunque haya error en algún hijo */
            exit(EXIT_FAILURE);
        }
        fprintf(in,"%d\n", 0);
        fclose(in);
        ...
        for(j=0; j<NUM_OPERATIONS; j++){
            ...
            while(Down_Semaforo(semid, i, SEM_UNDO) == ERROR){ /* Espera activa */
              ...
              if(active_wait == max_active_wait){ /* Control de errores: exceso de espera activa permitido */
                 ...
                 /* Los semaforos son liberados por el proceso padre aunque haya error en algún hijo */
                 /* No se levanta de nuevo el semaforo porque nunca se ha bajado */
                 free(name);
                 free(fcash);
                 exit(EXIT_FAILURE);
              }
           ...
           active_wait=0; /* Reseteamos la espera activa */
            ...
            if(!in){ /* Error */
                ...
                free(fcash);
                /* Los semaforos son liberados por el proceso padre aunque haya error en algún hijo */
                if(Up_Semaforo(semid, i, SEM_UNDO) == ERROR){ /* Levantamos antes de finalizar para no perjudicar al resto */
                   printf("Advertencia: no se ha podido levantar el semaforo despues del error anterior\n");
                }
                exit(EXIT_FAILURE);
            }
            fscanf(in, "%d\n", &amount);
            ...
            if(amount > MAX_AMOUNT){
               ...
               active_wait=0; /* Reseteamos la espera activa */
                ...
                if(!in){ /* Error */
                    ...
                    free(fcash);
                    /* Los semaforos son liberados por el proceso padre aunque haya error en algún hijo */
                    if(Up_Semaforo(semid, 0, SEM_UNDO) == ERROR){ /* Levantamos antes de finalizar para no perjudicar al resto */
                       printf("Advertencia: no se ha podido levantar el semaforo despues del error anterior\n");
                    }
                    exit(EXIT_FAILURE);
                }
    ...
    if(pid != 0){
        while(n_fin < num_cash){ /* Mientras el numero de cajas finalizadas sea menor que el numero de cajas total ... */
            ...
            if(!in){ /* Error */
                ...
                /* En este programa, no necesitamos levantar los semaforos, con borrarlos llega
                * porque si falla el proc. padre, los hijos no pueden hacer nada util */
                if(Borrar_Semaforo(semid) == ERROR){
                   printf("Advertencia: no se han podido eliminar los semaforos despues del error anterior\n");
                }
                exit(EXIT_FAILURE);
