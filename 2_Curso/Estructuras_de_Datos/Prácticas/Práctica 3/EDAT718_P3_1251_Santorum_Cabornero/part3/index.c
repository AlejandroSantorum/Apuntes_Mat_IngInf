...
int index_create(type_t type, char *path){
  ...
  if(type != INT && type != STR){
    ...
    printf("Incorrect type_t in index_create.\n.");
    exit(1);
  }
...
index_t* index_open(char* path){
  ...
  fread(&(idx->type), sizeof(type_t), 1, f);
  ...
  for(i = 0; i<idx->n_rec; i++){ /*Allocating and the reading the whole index (it is in the file) */
    ...
    fread(&(idx->list[i]->len), sizeof(int), 1, f);
    idx->list[i]->key = key_allocate(idx->list[i]->len);
    if(idx->list[i]->key == NULL){ /* Error */
    ...
    fread(idx->list[i]->key, 1, value_length_index(idx->list[i]->len), f); /* Reading the key */
...
int index_save(index_t *idx){
  ...
  for(i=0; i<idx->n_rec; i++){
    fwrite(&(idx->list[i]->len), 1, sizeof(int), f);
    fwrite(idx->list[i]->key, 1, value_length_index(idx->list[i]->len), f);
...
*/
int index_put(index_t *idx, void *key, long pos){
  int i;
  ...
  while(F <= L){
    ...
    if(value_cmp(idx->type, idx->list[M]->key, key) == 0){ /* We found the key so it was already in the index */
    ...
    else if(value_cmp(idx->type, idx->list[M]->key, key) > 0){
      L = M-1;
    }
    else{
      F = M+1;
  ...
  if(idx->type == INT){
    ...
    *((int *) ir->key) = *((int *) key);
    ...
    ir->key = strdup((char *) key);
    if(ir->key == NULL){
      free(ir);
      free(ir->ptrs);
      printf("Fatal Error. Index-F4-6.\n");
      exit(1);
    }
    ir->len = strlen((char *) key)+1;
    ir->ptrs[0] = pos;
    ir->n_ptrs = 1;
  }
...
/*
   ...
   using binary search.
*/
long* index_get(index_t *idx, void *key, int *nposs){
  int i;
  ...
  while(F <= L){
    ...
    if(value_cmp(idx->type, idx->list[M]->key, key) == 0){
