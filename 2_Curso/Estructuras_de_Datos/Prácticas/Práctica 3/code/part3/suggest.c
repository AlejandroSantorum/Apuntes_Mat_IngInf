...
int main(int argc, char **argv){
  ...
  array = index_get(idx, atoi(argv[1]), &n);
  if(array == NULL){ /* Error */
    printf("There is not any book with this score.\n");
    printf("We can't sugggest you any book.\n Try it again.\n");
    free(values);
    table_close(tb);
    index_close(idx);
    return EXIT_SUCCESS;
